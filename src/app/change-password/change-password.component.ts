...
export class ChangePasswordComponent implements OnInit{
  ...
  getUserDetails(){
    ...
    this.http.get("http://79.99.41.29/api/zrc/user/usercredentials"+"/"+this.auth.getUserId()).subscribe((resultData : any) => {
  ...
  onSaveClick(): void {
    ...
    this.http.put("http://79.99.41.29/api/zrc/user/update/usercredentails"+"/"+this.serial,bodyData).subscribe((resultData : any) => {
      if(resultData.status === 'false'){
  ...
  checkUsername(){
    ...
    this.http.get("http://79.99.41.29/api/zrc/register/getuser"+"/"+this.username).subscribe((resultData : any) =>{
      if(resultData.status === 'false'){
