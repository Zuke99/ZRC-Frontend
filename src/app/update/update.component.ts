...
export class UpdateComponent {
  ...
  //     this.http.get("http://79.99.41.29/api/zrc"+"/"+this.ph_number)
  ...
  getDetails() {
    this.http
      .get('http://79.99.41.29/api/zrc/indent' + '/' + this.serial)
      .subscribe((resultData: any) => {
  ...
  update(studentItem: any) {
    if (this.checkDate() && this.updatebalance(studentItem)) {
      ...
      this.http
        .put(
          'http://79.99.41.29/api/zrc/update' + '/' + studentItem.serial,
          bodydata
        )
        .subscribe((resultData: any) => {
          if(resultData. status === false){
